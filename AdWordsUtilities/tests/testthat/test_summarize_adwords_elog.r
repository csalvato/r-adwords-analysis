...
calculated_results <- AdWordsUtilities::summarize_elog(test_adwords_elog)

test_that("AdWordsUtilities::summarize_elog cost column is the total of all values in cost column", {
  expected_cost <- 35.67
  expect_equal(calculated_results$cost, expected_cost)
})

test_that("AdWordsUtilities::summarize_elog average_position column is the weighted mean of the average position (within 2 significant figures)", {
...
test_that("AdWordsUtilities::summarize_elog average_quality_score column is the average of all values in the quality_score column", {
  expected_average_quality_score <- 5
  expect_equal(calculated_results$average_quality_score, expected_average_quality_score)
})

test_that("AdWordsUtilities::summarize_elog estimated_available_impressions column is the sum of impressions over estimated impression share for each row", {
...
test_that("AdWordsUtilities::summarize_elog impressions column is the sum of all impressions", {
  expected_impressions <- 705
  expect_equal(calculated_results$impressions, expected_impressions)
})
test_that("AdWordsUtilities::summarize_elog est_search_impression_share column is total impressions over estimated available impressions", {
...
test_that("AdWordsUtilities::summarize_elog clicks column is the sum of all clicks", {
  expected_clicks <- 11
  expect_equal(calculated_results$clicks, expected_clicks)
})
test_that("AdWordsUtilities::summarize_elog click_through_rate column is sum of all clicks over sum of all impressions", {
  expected_click_through_rate <- 0.01560284
  expect_equal(calculated_results$click_through_rate, expected_click_through_rate)
})
test_that("AdWordsUtilities::summarize_elog num_acquisitions column is the count of distinct users purchasing in that time frame for the first time", {
...
test_that("AdWordsUtilities::summarize_elog conversion_rate column is the amount of people purchasing over the amount of clicks.", {
...
test_that("AdWordsUtilities::summarize_elog cost_per_click column is the sum of cost over the sum of clicks", {
  expected_cost_per_click <- signif(3.242727,3)
  expect_equal(signif(calculated_results$cost_per_click,3), expected_cost_per_click)
})
test_that("AdWordsUtilities::summarize_elog earnings column is the sum of all money paid to us", {
  expected_earnings <- 1515.83
  expect_equal(calculated_results$earnings, expected_earnings)
})
test_that("AdWordsUtilities::summarize_elog contribution column is earnings multiplied by margin", {
  expected_contribution <- signif(378.9575,3)
  expect_equal(signif(calculated_results$contribution,3), expected_contribution)
})
test_that("AdWordsUtilities::summarize_elog earnings_per_click column is sum of earnigns over the sum of clicks", {
...
test_that("AdWordsUtilities::summarize_elog contribution_per_click column is the sum of contribution over the sum of clicks.", {
...
test_that("AdWordsUtilities::summarize_elog cpa column is the cost of contribution over the cost of clicks.", {
  expected_cpa <- signif(2.547857,3)
  expect_equal(signif(calculated_results$cpa,3), expected_cpa)
})
test_that("AdWordsUtilities::summarize_elog referred_users column is the sum of all referred users", {
  expected_referred_users <- 0
  expect_equal(calculated_results$referred_users, expected_referred_users)
})
test_that("AdWordsUtilities::summarize_elog referred_earnings column is sum of all earnings by referees of referers", {
  expected_referred_earnings <- 0
  expect_equal(calculated_results$referred_earnings, expected_referred_earnings)
})
test_that("AdWordsUtilities::summarize_elog estimated_ltv column is number of acquisitions multipled estimated spend", {
  expected_estimated_ltv <- 2450
  expect_equal(calculated_results$estimated_ltv, expected_estimated_ltv)
})
test_that("AdWordsUtilities::summarize_elog estimated_lifetime_ROAS column is estimated spend over cost", {
...
test_that("AdWordsUtilities::summarize_elog total_earnings  column is the sum of referred and unreferred earnings", {
  expected_total_earnings  <- 1515.83
  expect_equal(calculated_results$total_earnings , expected_total_earnings )
})
test_that("AdWordsUtilities::summarize_elog total_contribution  column is the sum of referred and unreferred contribution", {
  expected_total_contribution  <- 378.9575
  expect_equal(calculated_results$total_contribution , expected_total_contribution )
})
test_that("AdWordsUtilities::summarize_elog actual_ROAS  column is the return on ad spend (total_contribution - cost)/cost", {
