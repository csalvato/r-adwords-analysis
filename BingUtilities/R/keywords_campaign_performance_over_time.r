...
keywords_campaign_performance_over_time <- function(keywords_elog, plot = TRUE, plot_png_file=NULL){
  require(plyr)
  require(dplyr)
  ...
  keywords_campaigns_over_time <- keywords_elog %>%
	...
	if( plot ) {
		require(ggplot2)
		the_plot  <- ggplot(keywords_campaigns_over_time %>% filter(keyword == "paleo meals"), aes(week,value,group=type,col=type,fill=type)) + 
					       geom_line() + 
